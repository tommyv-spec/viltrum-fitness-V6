...
function loadUserData(username) {
  ...
  // ⚡ FIRST: Try to use SessionCache data (already loaded on dashboard!)
  if (typeof window.SessionCache !== 'undefined') {
    const sessionData = sessionStorage.getItem('viltrum_session_data');
    if (sessionData) {
      try {
        const data = JSON.parse(sessionData);
        console.log('✅ Using pre-loaded SessionCache data (loaded on dashboard)');
  ...
  // THIRD: Fetch fresh data (only if no cache available)
